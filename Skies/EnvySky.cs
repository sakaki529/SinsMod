...
namespace SinsMod.Skies
{
    public class EnvySky : CustomSky
    {
        ...
        {
            RuneTexture = mod.GetTexture("Extra/Rune/EnvyRune");
        }
        private float GetIntensity()
        {
            if (UpdatePIndex())
            {
                ...
                if (EnvyIndex != -1)
                {
                    x = Vector2.Distance(Main.player[Main.myPlayer].Center, Main.npc[EnvyIndex].Center);
                }
        ...
        public override Color OnTileColor(Color inColor)
        {
            ...
            return new Color(Vector4.Lerp(new Vector4(0.3f, 0.8f, 0.3f, 1f), inColor.ToVector4(), 1f - Intensity));
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= 0f && minDepth < 0f)
            {
                ...
                //spriteBatch.Draw(Main.blackTileTexture, new Rectangle(0, 0, Main.screenWidth, Main.screenHeight), new Color(0, 80, 0) * Intensity);
            }
