...
{
    public class RedHeal : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                float num8 = projectile.velocity.X * 0.334f * num7;
                float num9 = -(projectile.velocity.Y * 0.334f) * num7;
                int num10 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 183, 0f, 0f, 100, default(Color), 1.0f);
            ...
            {
                float num12 = projectile.velocity.X * 0.2f * num11;
                float num13 = -(projectile.velocity.Y * 0.2f) * num11;
                int num14 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 183, 0f, 0f, 100, default(Color), 0.8f);
                Main.dust[num14].noGravity = true;
                Main.dust[num14].velocity *= 0f;
                Dust dust = Main.dust[num14];
                dust.position.X = dust.position.X - num12;
                Dust dust2 = Main.dust[num14];
                dust2.position.Y = dust2.position.Y - num13;
            }
        }
    }
}
