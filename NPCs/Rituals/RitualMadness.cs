...
{
    public class RitualMadness : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (npc.ai[1] == 180f && !NPC.AnyNPCs(mod.NPCType("BlackCrystalNoMove")) && !NPC.AnyNPCs(mod.NPCType("BlackCrystal")) && !NPC.AnyNPCs(mod.NPCType("BlackCrystalCore")))
            {
                NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y + 48, mod.NPCType("BlackCrystalNoMove")); ;
            }
            ...
            {
                int num1;
                for (int num2 = 0; num2 < 2; num2 = num1 + 1)
                {
                    ...
                    vector2.Normalize();
                    int num5 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 186, 0f, 0f, 50, default(Color), 1.5f);
                    Main.dust[num5].noGravity = true;
                    Main.dust[num5].noLight = true;
                    Main.dust[num5].position = npc.Center + vector2 * 127f * num4;
                    Main.dust[num5].shader = GameShaders.Armor.GetSecondaryShader(56, Main.LocalPlayer);
            ...
            {
                int num1;
                for (int num2 = 0; num2 < 1; num2 = num1 + 1)
                {
                    float scaleFactor = Main.rand.Next(1, 3);
                    Vector2 vector2 = new Vector2(Main.rand.Next(-10, 11), Main.rand.Next(-10, 11));
                    vector2.Normalize();
                    int num3 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 186, 0f, 0f, 50, default(Color), 1.5f);
