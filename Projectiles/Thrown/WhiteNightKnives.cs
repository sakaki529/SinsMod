...
{
    public class WhiteNightKnives : ModProjectile
	{
        ...
        public override void AI()
        {
            ...
            {
                int d = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 247, projectile.velocity.X * 0.1f, projectile.velocity.Y * 0.1f, 0, new Color(255, 255, 255, 100), 0.8f);
            ...
            if (projectile.ai[0] >= 15)
            {
                ...
                if (flag)
                {
                    ...
                    num9 *= num11;
                    num10 *= num11;
                    projectile.velocity.X = (projectile.velocity.X * 5f + num9) / 6f;
                    projectile.velocity.Y = (projectile.velocity.Y * 5f + num10) / 6f;
                }
            }
        }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                int num2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 247, -projectile.velocity.X * 0.2f, -projectile.velocity.Y * 0.2f, 0, default(Color), 0.6f);
                Main.dust[num2].noGravity = true;
                Dust dust = Main.dust[num2];
                dust.velocity *= 0.6f;
                num2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 247, -projectile.velocity.X * 0.2f, -projectile.velocity.Y * 0.2f, 0, default(Color), 0.6f);
                dust = Main.dust[num2];
                dust.noGravity = true;
                dust.velocity *= 0.6f;
            }
