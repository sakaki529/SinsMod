...
namespace SinsMod.Projectiles.Melee
{
    public class TerraBall : ModProjectile
    {
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            {
                Vector2 drawOff = projectile.velocity * i * 1.5f;
                float strength = 0.4f - i * 0.06f;
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 0; i < 4; i++)
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 107, 0f, 0f, 100, default(Color), 0.9f);
