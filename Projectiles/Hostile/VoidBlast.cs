    ...
    public class VoidBlast : ModProjectile
    {
        ...
        private void AICharge()
        {
            ...
            if (projectile.ai[0] == 0) //initial firing
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    int d = Dust.NewDust(projectile.position + new Vector2(projectile.width / 2, projectile.height / 2) + projectile.velocity * muzzleDist, 1, 1, 66, 0f, 0f, 100, Color.Purple, 1.5f);
                    Main.dust[d].noGravity = true;
                    Main.dust[d].velocity *= 0.4f;
                    d = Dust.NewDust(endPos + new Vector2(projectile.width / 2, projectile.height / 2), 0, 0, 66, 0f, 0f, 100, Color.Purple, 1f);
                    Main.dust[d].noGravity = true;
                    Main.dust[d].velocity *= 0.4f;
                }
            }
        ...
        private void AILaunch()
        {
            ...
            if (projectile.damage > 1 && projectile.timeLeft % 2 == 0)
            {
                projectile.damage -= 1;
            }
            ...
            if (projectile.ai[1] > 0.12f)
            {
                projectile.ai[1] *= 0.98f;
            }
            //purpel stuph
            int d = Dust.NewDust(projectile.position + new Vector2(projectile.width / 2f, projectile.height / 2f), 1, 1, 66, 0f, 0f, 100, Color.Purple, 1f);
            Main.dust[d].noGravity = true;
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 0; i < 40; i++)
            {
                int d = Dust.NewDust(projectile.position, projectile.width, projectile.height, 66, (i / 90f) * -projectile.velocity.X, (i / 90f) * -projectile.velocity.Y, 100, Color.Purple, 1.2f);
                Main.dust[d].noGravity = true;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            drawLaser(spriteBatch, center);
            spriteBatch.Draw(texture, projectile.position - Main.screenPosition + center, new Rectangle?(new Rectangle(0, 0, texture.Width, texture.Height)), Color.White, projectile.rotation, center, projectile.scale, SpriteEffects.None, 0);
            return false;
        }
        private void drawLaser(SpriteBatch spritebatch, Vector2 centre)
        {
            ...
            if (projectile.ai[0] > 0)
            {
                ...
                Utils.DrawLaser(spritebatch, mod.GetTexture("Extra/Projectile/VoidBlast_Beam"), start - Main.screenPosition, end - Main.screenPosition, new Vector2(projectile.ai[1]), new Utils.LaserLineFraming(Laser)); //uses delegate (see method below)
            }
        }
