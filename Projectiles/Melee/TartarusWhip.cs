...
{
    public class TartarusWhip : ModProjectile
	{
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            rectangle = new Rectangle(0, 88, texture2D.Width, 28);//Head
        ...
        public override void AI()
        {
            ...
            if (projectile.alpha == 0)
            {
                /*for (int i = 0; i < 2; i++)
                {
                    Dust dust = Main.dust[Dust.NewDust(projectile.position + projectile.velocity * 2f, projectile.width, projectile.height, 21, 0f, 0f, 100, new Color(255, 255, 255, 100), 1.0f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                for (int i = 0; i < 2; i++)
                {
                    Vector2 value = projectile.position + projectile.velocity * 2f + (projectile.rotation - 1.57079637f).ToRotationVector2() * 8f;
                    Vector2 value2 = projectile.rotation.ToRotationVector2() * 16f;
                    Dust dust = Main.dust[Dust.NewDust(value + value2, 0, 0, 21, 0, 0, 100, Color.Transparent, 1f + Main.rand.NextFloat() * 1.2f)];
                    dust.noGravity = true;
                    dust.noLight = true;
                    dust.position -= new Vector2(4f);
                    dust.fadeIn = 1f;
                    dust.velocity += projectile.localAI[0].ToRotationVector2();
                    Dust dust2 = Main.dust[Dust.NewDust(value - value2, 0, 0, 21, 0, 0, 100, Color.Transparent, 1f + Main.rand.NextFloat() * 1.2f)];
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            target.AddBuff(mod.BuffType("AbyssalFlame"), 240);
            if (projectile.localAI[1] <= 0f && projectile.owner == Main.myPlayer)
            {
                //Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("AbyssalBurst"), damage, 1f, projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
            }
