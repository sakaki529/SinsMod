...
{
    public class LastNemesisLaser : ModProjectile
	{
        public override string Texture => "SinsMod/Projectiles/Magic/LightOfEdenLaser";
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            float prismHue = 0.75f + (float)Math.Cos(Main.time / 180.0 * 6.2831854820251465) * 0.07f;
            ...
            DelegateMethods.c_1 = color * 0.75f * projectile.Opacity; //outside
            ...
            Utils.DrawLaser(Main.spriteBatch, tex, value - Main.screenPosition, value + projectile.velocity * num - Main.screenPosition, vector, new Utils.LaserLineFraming(DelegateMethods.RainbowLaserDraw)); //outside
            DelegateMethods.c_1 = new Color(255, 255, Main.DiscoB + 125, 127) * 0.75f * projectile.Opacity; //inside
            Utils.DrawLaser(Main.spriteBatch, tex, value - Main.screenPosition, value + projectile.velocity * num - Main.screenPosition, vector / 2f, new Utils.LaserLineFraming(DelegateMethods.RainbowLaserDraw)); //inside
            return false;
        }
        public override void AI()
        {
            ...
            if (!proj.active || proj.type != mod.ProjectileType("LastNemesis"))
            {
                projectile.Kill();
            }
            ...
            if (proj.ai[0] < 180f)
            {
                ...
                scaleFactor2 = 1f - proj.ai[0] / 180f;
            }
            else
            {
                scaleFactor2 = 0f;
            ...
            projectile.scale = 1.4f * (1f - scaleFactor2);
            projectile.damage = proj.damage;
            if (proj.ai[0] >= 180f)
            {
                projectile.damage = (int)((double)projectile.damage * 2.0f);
            ...
            if (Math.Abs(projectile.localAI[1] - num9) < 100f && projectile.scale > 0.15f)
            {
                ...
                float x = Main.rgbToHsl(new Color(prismHue, 1f, 0.5f)).X;
                for (int num11 = 0; num11 < 2; num11++)
                {
                    ...
                    int d = Dust.NewDust(vector2, 0, 0, 267, vector3.X, vector3.Y, 0, default(Color), 1f);
                    ...
                    Main.dust[d].color = Color.Lerp(color, Main.hslToRgb(prismHue, 1f, 0.5f), projectile.scale / 1.4f);
                }
                ...
                if (Main.netMode != 2)
                {
                    ((WaterShaderData)Filters.Scene["WaterDistortion"].GetShader()).QueueRipple(projectile.position + new Vector2(size.X * 0.5f, 0f).RotatedBy(num14, default(Vector2)), new Color(0.5f, 0.1f * (float)Math.Sign(value4) + 0.5f, 0f, 1f) * Math.Abs(value4), size, RippleShape.Square, num14);
                }
