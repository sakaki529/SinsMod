...
namespace SinsMod.NPCs.Boss.Sins.Pride
{
    [AutoloadBossHead]
    public class Pride : ModNPC
    {
            ...
            if (npc.ai[3] == 0f)
            {
                for (int x = 0; x < size; x++)
                {
                    for (int y = 0; y < size; y++)
                    {
                        ...
                        spriteBatch.Draw(mod.GetTexture("NPCs/Boss/Sins/Pride/Particle"), drawPos, null, npc.GetAlpha(Color.White) * aura[x, y], 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
                    }
                }
            }
            else
        ...
        public override bool PreAI()
        {
            ...
            if (npc.ai[3] > 0f)
            {
                ...
                if (Main.rand.NextBool(5) && npc.ai[3] < 120f)
                {
                    ...
                    {
                        Dust dust = Main.dust[Dust.NewDust(npc.Left, npc.width, npc.height / 2, 242, 0f, 0f, 0, SinsColor.Pride, 1f)];
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("PrideTrophy"));
            }
            if (Main.expertMode)
            {
                npc.DropBossBags();
            ...
            else
            {
                ...
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("EssenceOfPride"), Main.rand.Next(4, 9));
                switch (Main.rand.Next(3))
                {
                    case 0:
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("Proudia"));
                        break;
                    case 1:
                        Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType(""));
                        break;
