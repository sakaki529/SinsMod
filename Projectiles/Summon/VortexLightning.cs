...
{
    public class VortexLightning : ModProjectile
    {
        public override string Texture => "SinsMod/Projectiles/Hostile/LightningArc";
        ...
        public override void AI()
        {
            ...
            if (projectile.velocity == Vector2.Zero)
            {
                if (projectile.frameCounter >= projectile.extraUpdates * 2)
                {
                    ...
                    bool flag = true;
                    for (int num2 = 1; num2 < projectile.oldPos.Length; num2 = num + 1)
                    {
                        if (projectile.oldPos[num2] != projectile.oldPos[0])
                        {
                            flag = false;
                        }
                        num = num2;
                ...
                {
                    for (int num3 = 0; num3 < 2; num3 = num + 1)
                    {
                        float num4 = projectile.rotation + ((Main.rand.Next(2) == 1) ? -1f : 1f) * 1.57079637f;
                        float num5 = (float)Main.rand.NextDouble() * 0.8f + 1f;
                        Vector2 vector = new Vector2((float)Math.Cos(num4) * num5, (float)Math.Sin(num4) * num5);
                        int num6 = Dust.NewDust(projectile.Center, 0, 0, 226, vector.X, vector.Y, 0, default(Color), 1f);
                    ...
                    if (Main.rand.Next(5) == 0)
                    {
                        ...
                        int num7 = Dust.NewDust(projectile.Center + value - Vector2.One * 4f, 8, 8, 31, 0f, 0f, 100, default(Color), 1.5f);
            ...
            else
            {
                if (projectile.frameCounter >= projectile.extraUpdates * 2)
                {
                    ...
                    do
                    {
                        ...
                        num10 %= 100;
                        float num11 = num10 / 100f * 6.28318548f;
                        vector3 = num11.ToRotationVector2();
                        if (vector3.Y > 0f)
                        {
                            vector3.Y *= -1f;
                        }
                        ...
                        if (vector3.X * (projectile.extraUpdates + 1) * 2f * num8 + projectile.localAI[0] > 40f)
                        {
                            flag2 = true;
                        }
                        if (vector3.X * (projectile.extraUpdates + 1) * 2f * num8 + projectile.localAI[0] < -40f)
                ...
                IL_2:
                    if (projectile.velocity != Vector2.Zero)
                    {
                        projectile.localAI[0] += vector2.X * (projectile.extraUpdates + 1) * 2f * num8;
                        projectile.velocity = vector2.RotatedBy(projectile.ai[0] + 1.57079637f, default(Vector2)) * num8;
                        projectile.rotation = projectile.velocity.ToRotation() + 1.57079637f;
                        if (Main.rand.Next(4) == 0 && Main.netMode != 1 && projectile.localAI[1] == 0f)
                        {
                            float num12 = Main.rand.Next(-3, 4) * 1.04719758f / 3f;
                            Vector2 vector4 = projectile.ai[0].ToRotationVector2().RotatedBy(num12, default(Vector2)) * projectile.velocity.Length();
                            if (!Collision.CanHitLine(projectile.Center, 0, 0, projectile.Center + vector4 * 50f, 0, 0))
                            {
                                Projectile.NewProjectile(projectile.Center.X - vector4.X, projectile.Center.Y - vector4.Y, vector4.X, vector4.Y, projectile.type, projectile.damage, projectile.knockBack, projectile.owner, vector4.ToRotation() + 1000f, projectile.ai[1]);
                                return;
                            }
                        }
                    }
