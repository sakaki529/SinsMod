...
namespace SinsMod.Skies
{
    public class WrathSky : CustomSky
    {
        ...
        {
            RuneTexture = mod.GetTexture("Extra/Rune/WrathRune");
        }
        private float GetIntensity()
        {
            if (UpdatePIndex())
            {
                ...
                if (WrathIndex != -1)
                {
                    x = Vector2.Distance(Main.player[Main.myPlayer].Center, Main.npc[WrathIndex].Center);
                }
        ...
        public override Color OnTileColor(Color inColor)
        {
            ...
            return new Color(Vector4.Lerp(new Vector4(0.6f, 0.1f, 0.1f, 1f), inColor.ToVector4(), 1f - Intensity));
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= 0f && minDepth < 0f)
            {
                ...
                //spriteBatch.Draw(Main.blackTileTexture, new Rectangle(0, 0, Main.screenWidth, Main.screenHeight), new Color(105, 0, 0) * Intensity);
            }
