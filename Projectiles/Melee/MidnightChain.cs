...
{
    public class MidnightChain : ModProjectile
	{
        ...
        public override void AI()
        {
            ...
            if (projectile.alpha == 0)
            {
                for (int i = 0; i < 2; i++)
                {
                    Dust dust = Main.dust[Dust.NewDust(projectile.position + projectile.velocity * 2f, projectile.width, projectile.height, 27, 0f, 0f, 100, default(Color), 1.0f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                while (num4 < num3)
                {
                    if (Main.rand.Next(4) == 0)
                    {
                        ...
                        Dust dust = Main.dust[Dust.NewDust(position, projectile.width, projectile.height, 27, 0f, 0f, 100, default(Color), 1.2f)];
                        dust.noGravity = true;
                        dust.fadeIn = 0.5f;
                        dust.velocity += projectile.localAI[0].ToRotationVector2();
                        dust.noLight = true;
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("MidnightBurst"), projectile.damage, projectile.knockBack, projectile.owner, target.whoAmI, 0f);
            }
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            ...
            if (projectile.localAI[1] <= 0f && projectile.owner == Main.myPlayer)
            {
                Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("MidnightBurst"), projectile.damage, projectile.knockBack, projectile.owner, target.whoAmI, 1f);
            }
            projectile.localAI[1] = 4f;
        }
