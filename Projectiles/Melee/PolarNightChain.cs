...
{
    public class PolarNightChain : ModProjectile
	{
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Rectangle rectangle = new Rectangle(0, 0, texture2D.Width, 42);//Grip
            Vector2 vector2 = new Vector2(0f, Main.player[projectile.owner].gfxOffY);
            float num2 = projectile.rotation + 3.14159274f;
            Main.spriteBatch.Draw(texture2D, projectile.Center.Floor() - Main.screenPosition + vector2, new Rectangle?(rectangle), alpha, num2, rectangle.Size() / 2f - Vector2.UnitY * 4f, projectile.scale, 0, 0f);
            num -= 40f * projectile.scale;
            Vector2 vector3 = projectile.Center.Floor();
            vector3 += vector * projectile.scale * 24f;
            rectangle = new Rectangle(0, 68, texture2D.Width, 18);//Middle by Head
            if (num > 0f)
            {
                float num3 = 0f;
                while (num3 + 1f < num)
                {
                    ...
                    Main.spriteBatch.Draw(texture2D, vector3 - Main.screenPosition + vector2, new Rectangle?(rectangle), alpha, num2, new Vector2(rectangle.Width / 2, 0f), projectile.scale, 0, 0f);
            ...
            vector3 += vector * projectile.scale * 24f;
            rectangle = new Rectangle(0, 46, texture2D.Width, 18);//Middle by Grip
            int num4 = 10;
            if (flag)
            {
                num4 = 5;
            ...
            rectangle = new Rectangle(0, 90, texture2D.Width, 32);//Head
            Main.spriteBatch.Draw(texture2D, vector4 - Main.screenPosition + vector2, new Rectangle?(rectangle), alpha, num2, texture2D.Frame(1, 1, 0, 0).Top(), projectile.scale, 0, 0f);
            return false;
        }
        public override void AI()
        {
            ...
            Vector2 vector = (num2 * (projectile.ai[0] / 30f * 6.28318548f - 1.57079637f)).ToRotationVector2();//time
            ...
            projectile.ai[0] += 1f;
            if (projectile.ai[0] < 30f)//active time
            {
                projectile.velocity += 20f * vector;//length
            }
            else
            {
                projectile.Kill();
            ...
            if (projectile.alpha == 0)
            {
                for (int i = 0; i < 2; i++)
                {
                    Dust dust = Main.dust[Dust.NewDust(projectile.position + projectile.velocity * 2f, projectile.width, projectile.height, 186, 0f, 0f, 100, new Color(255, 255, 255, 100), 0.8f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                while (num4 < num3)
                {
                    if (Main.rand.Next(4) == 0)
                    {
                        ...
                        Dust dust = Main.dust[Dust.NewDust(position, projectile.width, projectile.height, 186, 0f, 0f, 100, new Color(255, 255, 255, 100), 1.0f)];
                        dust.noGravity = true;
                        dust.fadeIn = 0.5f;
                        dust.velocity += projectile.localAI[0].ToRotationVector2();
                        dust.noLight = true;
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                //Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("TidalExplosion"), projectile.damage, projectile.knockBack, projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
            }
