...
namespace SinsMod.NPCs.Boss.Sins.Lust
{
    ...
    public class Lust : ModNPC
    {
            ...
            if (npc.ai[3] == 0f)
            {
                for (int x = 0; x < size; x++)
                {
                    for (int y = 0; y < size; y++)
                    {
                        ...
                        spriteBatch.Draw(mod.GetTexture("NPCs/Boss/Sins/Lust/Particle"), drawPos, null, npc.GetAlpha(Color.White) * aura[x, y], 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
                    }
                }
            }
            else
        ...
        public override bool PreAI()
        {
            ...
            if (npc.ai[3] > 0f)
            {
                ...
                if (Main.rand.NextBool(5) && npc.ai[3] < 120f)
                {
                    ...
                    {
                        Dust dust = Main.dust[Dust.NewDust(npc.Left, npc.width, npc.height / 2, 242, 0f, 0f, 0, SinsColor.Lust, 1f)];
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("LustTrophy"));
            }
            if (Main.expertMode)
            {
                npc.DropBossBags();
            ...
            else
            {
                ...
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("EssenceOfLust"), Main.rand.Next(4, 9));
