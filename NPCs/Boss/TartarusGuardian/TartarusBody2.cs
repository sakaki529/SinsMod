...
{
    [AutoloadBossHead]
    public class TartarusBody2 : ModNPC
    {
        private bool Spawn;
        ...
        public override bool PreAI()
        {
            if (!Spawn)
            {
                int num = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y + 15, mod.NPCType("TartarusWings"), npc.whoAmI, npc.whoAmI);
                Main.npc[num].alpha = npc.alpha;
                Spawn = true;
            }
            return base.PreAI();
        }
