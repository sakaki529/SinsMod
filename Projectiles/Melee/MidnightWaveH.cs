...
{
    public class MidnightWaveH : ModProjectile
    {
        public override string Texture => "SinsMod/Projectiles/Melee/MidnightWave";
        ...
        public override void AI()
        {
            for (int i = 0; i < 1; i++)
            {
                ...
                Dust dust = Dust.NewDustDirect(projectile.Center, 0, 0, 181, 0f, 0f, 0, default(Color), 0.6f);
                dust.alpha = 127;
                dust.fadeIn = 0.7f;
                dust.velocity *= 0.3f;
                dust.position = projectile.Center + vector;
                dust.noGravity = true;
                dust.shader = GameShaders.Armor.GetSecondaryShader(72, Main.LocalPlayer);
            }
            projectile.ai[0] += 1f;
            ...
            if (target)
            {
                ...
                projectile.velocity = (5 * projectile.velocity + move) / 5f;
                AdjustMagnitude(ref projectile.velocity);
            }
        }
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("MidnightSword"), projectile.damage, 0, projectile.owner, target.whoAmI, 0);
            if (!Main.player[projectile.owner].moonLeech && projectile.numHits == 0)
            {
                if (target.type != 488)
                {
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            target.AddBuff(BuffID.Bleeding, 300);
            target.AddBuff(BuffID.ShadowFlame, 300);
            Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("MidnightSword"), projectile.damage, 0, projectile.owner, target.whoAmI, 1);
        }
        ...
        public override void Kill(int timeleft)
		{
            ...
            for (int i = 0; i < num; i++)
            {
                int num2 = Dust.NewDust(projectile.Center, 0, 0, 181, 0f, 0f, 100, default(Color), 0.8f);
                Dust dust = Main.dust[num2];
                dust.velocity *= 4f * (0.3f + 0.7f * Main.rand.NextFloat());
                Main.dust[num2].fadeIn = 0.8f + Main.rand.NextFloat() * 0.2f;
                Main.dust[num2].noGravity = true;
                Main.dust[num2].shader = GameShaders.Armor.GetSecondaryShader(72, Main.LocalPlayer);
                dust = Main.dust[num2];
