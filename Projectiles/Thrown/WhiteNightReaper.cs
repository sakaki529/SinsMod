...
{
    public class WhiteNightReaper : ModProjectile
	{
        private bool Spawn;
        private int Count;
        public override string Texture => "SinsMod/Items/Weapons/MultiType/WhiteNightReaper";
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Texture2D glowTexture = mod.GetTexture("Glow/Item/WhiteNightReaper_Glow");
        ...
        public override void AI()
        {
            ...
            int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 247, 0f, 0f, 0, default(Color), 1f);
            Main.dust[dust].noGravity = true;
            Main.dust[dust].velocity = Utils.NextFloat(Main.rand, 0.3f, 0.6f) * projectile.velocity;
            Main.dust[dust].scale = 0.7f + Utils.NextFloat(Main.rand, 0.3f);
            projectile.alpha -= 15;
            ...
            if (projectile.localAI[0] == 0f)
            {
                ...
                if (projectile.localAI[1] >= 50f || projectile.numHits > 1)
                {
            ...
            if (!Spawn)
            {
                Count = Main.rand.Next(16, 24);
                Spawn = true;
            }
            if (Count < 0 && Spawn)
            {
                float num6 = (projectile.direction <= 0) ? -1f : 1f;
                Vector2 vector = projectile.Center + new Vector2(Main.rand.Next(0, 10), Main.rand.Next(0, 10));
                if (projectile.owner == Main.myPlayer)
                {
                    Projectile.NewProjectile(vector, Vector2.Zero, mod.ProjectileType("WhiteNightReaperBlade"), projectile.damage / 3, 1f, projectile.owner, projectile.ai[0], num6);
                }
                Count = Main.rand.Next(24, 36);
            }
