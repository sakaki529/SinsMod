...
{
    public class MysticSpray : ModProjectile
	{
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Mystic Spray");
        }
        ...
        {
            //int dustType = mod.DustType("MysticSolution");
            int dustType = 88;
            ...
            for (int k = i - size; k <= i + size; k++)
            {
                for (int l = j - size; l <= j + size; l++)
                {
                    if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < 6)
                    {
                        ...
                        {
                            Main.tile[k, l].type = (ushort)mod.TileType("MysticGrass");
                            WorldGen.SquareTileFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);
                        }
                        else if (TileID.Sets.Conversion.Stone[type])
                        {
                            Main.tile[k, l].type = (ushort)mod.TileType("MysticStone");
                            WorldGen.SquareTileFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);
                        }
                        if (WallID.Sets.Conversion.Grass[wall])
                        {
                            Main.tile[k, l].wall = (ushort)mod.WallType("MysticGrassWall");
                            WorldGen.SquareWallFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);
                        }
                        else if (WallID.Sets.Conversion.Stone[wall])
                        {
                            Main.tile[k, l].wall = (ushort)mod.WallType("MysticStoneWall");
                            WorldGen.SquareWallFrame(k, l, true);
                            NetMessage.SendTileSquare(-1, k, l, 1);
                        }
                    }
