...
{
    public class LunarEyeSmall : ModNPC
    {
        private bool Esc;
        private int EscTimer;
        public override string Texture => "SinsMod/NPCs/Boss/LunarEye/LunarEye";
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    Gore.NewGore(npc.position, npc.velocity * num * Main.rand.NextFloat(), mod.GetGoreSlot("Gores/Boss/LunarEye2"), j > 0 ? npc.scale * 0.9f : npc.scale);
                }
                for (int k = 0; k < 30; k++)
                {
                    Dust.NewDust(npc.position, npc.width, npc.height, 209, 0f, 0f, 0, default(Color), 0.7f);
                }
            }
        }
        public override void AI()
        {
            ...
            if (!NPC.AnyNPCs(mod.NPCType("LunarEye")))
            {
                npc.life = 0;
                npc.HitEffect(0, 1);
                npc.active = false;
            }
            if (Main.rand.Next(840) == 0)
            {
            ...
            while (num1243 < 10)
            {
                num1241 = (float)NPC.MoonLordAttacksArray2[1, num1243];
                if (num1241 + (float)num1244 > npc.ai[1])
                {
                    break;
                }
                num1244 += (int)num1241;
            ...
            npc.ai[0] = (float)NPC.MoonLordAttacksArray2[0, num1243];
            float num1245 = (float)((int)npc.ai[1] - num1244);
            if (npc.ai[0] != num1242)
            {
                npc.netUpdate = true;
            }
            ...
            if (npc.ai[0] == 0f)
            {
                ...
                {
                    if (num1248 != npc.whoAmI && Main.npc[num1248].active && Main.npc[num1248].type == mod.NPCType("LunarEye") && Vector2.Distance(npc.Center, Main.npc[num1248].Center) < 150f)
                    {
            ...
            if (npc.ai[0] == 1f)
            {
                ...
                if ((num1245 == num1241 - 14f || num1245 == num1241 - 7f || num1245 == num1241) && Main.netMode != 1)
                {
                    ...
                    Projectile.NewProjectile(npc.Center.X + vector214.X, npc.Center.Y + vector214.Y, vector215.X, vector215.Y, 462, 25, 0f, Main.myPlayer, 0f, 0f);
                    return;
                }
            }
            else
                ...
                if (npc.ai[0] == 2f)
                {
                    ...
                    else
                    {
                        if (num1245 < 75f)
                        {
                            ...
                            Vector2 value25 = spinningpoint2.RotatedBy((double)((float)num1250 * 6.28318548f / 6f), default(Vector2));
                            Vector2 value26 = spinningpoint2.RotatedBy((double)((float)num1251 * 6.28318548f / 6f), default(Vector2));
                            ...
                            if ((num1245 - 15f) % 10f == 0f && Main.netMode != 1)
                            {
                                ...
                                vector217 *= 4f;
                                int num1254 = Projectile.NewProjectile(npc.Center.X + vector216.X, npc.Center.Y + vector216.Y, vector217.X, vector217.Y, 454, 40, 0f, Main.myPlayer, 30f, npc.whoAmI);
                                return;
                            }
                        }
                        else
                ...
                else
                {
                    if (npc.ai[0] == 3f)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                npc.localAI[0] = 0f;
                                npc.localAI[1] = (float)Math.Sin((double)((num1245 - 15f) * 6.28318548f / 15f)) * 0.5f;
                                if (npc.localAI[1] < 0f)
                                {
                                    npc.localAI[0] = 3.14159274f;
                                    return;
                            ...
                            else
                            {
                                ...
                                if ((num1245 - 15f - 30f) % 10f == 0f && Main.netMode != 1)
                                {
                                    ...
                                    Projectile.NewProjectile(vector219.X, vector219.Y, vector220.X, vector220.Y, 452, 30, 0f, Main.myPlayer, 0f, ai3);
                                    return;
                                }
                            }
                        }
                            ...
                            if (num1245 < 180f)
                            {
                                ...
                                if (num1245 >= 60f)
                                {
                                    ...
                                    for (int num1258 = 0; num1258 < 1 + num1257; num1258 = num + 1)
                                    {
                                        ...
                                        Main.dust[num1261].velocity = Vector2.Normalize(center25 - vector221) * 3.5f * (10f - (float)num1257 * 2f) / 10f;
                            ...
                            else
                            {
                                if (num1245 < num1241 - 15f)
                                {
                                    if (num1245 == 180f && Main.netMode != 1)
                                    {
                                        ...
                                        Projectile.NewProjectile(npc.Center.X, npc.Center.Y, vector222.X, vector222.Y, mod.ProjectileType("PhantasmalDeathray"), 50, 0f, Main.myPlayer, num1262 * 6.28318548f / 540f, npc.whoAmI);
                                    ...
                                    float num1263 = (float)(npc.ai[2] >= 0f).ToDirectionInt();
