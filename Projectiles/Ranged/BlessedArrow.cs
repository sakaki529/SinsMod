...
{
    public class BlessedArrow : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Blessed Arrow");
        }
        ...
        public override void AI()
        {
            ...
            if (projectile.alpha < 0)
            {
                ...
                while (num2 < num)
                {
                    ...
                    int num3 = Dust.NewDust(projectile.Center, 0, 0, 246, 0f, 0f, 0, default(Color), 1.5f);
            ...
            {
                Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 60, 1f, 0f);
                projectile.ai[0] = 1f;
            }
            if (projectile.ai[0] == 1f)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    int num7 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 246, 0f, 0f, 0, default(Color), 1.0f);
            ...
            if (projectile.localAI[1] == 15f)
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    int num8 = Dust.NewDust(projectile.Center, 0, 0, 246, 0f, 0f, 160, default(Color), 1.1f);
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, mod.ProjectileType("BlessedExplosion"), projectile.damage / 2, projectile.knockBack, projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
        }
        ...
        {
            Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0f, 0f, mod.ProjectileType("BlessedExplosion"), projectile.damage / 2, projectile.knockBack, projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                num *= num4;
                num2 *= num4;
                int num5 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 246, 0f, 0f, 100, default(Color), 3.0f);
